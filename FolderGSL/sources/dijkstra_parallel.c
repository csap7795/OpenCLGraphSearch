...
void dijkstra_parallel_cpu(Graph* graph, unsigned source, unsigned device_num, cl_float* out_cost, cl_uint* out_path, unsigned long *time) // bool* check_cycles, bool* negative_cycles,
{
    /*First, build the program*/
    build_program(device_num);
    ...
    // Create Memory Buffers for Graph Data
    cl_mem vertice_buffer = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR ,sizeof(cl_uint) * (graph->V+1), graph->vertices, &err);
    CLU_ERRCHECK(err,"Failed creating verticebuffer");

    cl_mem edge_buffer = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,sizeof(cl_uint) * graph->E, graph->edges, &err);
    CLU_ERRCHECK(err,"Failed creating edgebuffer");

    cl_mem weight_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(cl_float) * graph->E,graph->weight, &err);
    CLU_ERRCHECK(err,"Failed creating weight_buffer");
    ...
    cl_mem cost_buffer = clCreateBuffer(context,CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR ,sizeof(cl_float) * (graph->V), out_cost, &err);
    CLU_ERRCHECK(err,"Failed creating cost_buffer");

    cl_mem predecessor_buffer = clCreateBuffer(context,CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,sizeof(cl_uint) * (graph->V), out_path, &err);
    CLU_ERRCHECK(err,"Failed creating predecessor_buffer");
