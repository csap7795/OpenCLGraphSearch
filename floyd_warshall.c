...
void parallel_floyd_warshall_global(cl_float** in_matrix, cl_float** out_matrix, cl_uint** out_path, unsigned length, size_t device_num)
{
    ...
    printf("Time for parallel approach : %lu ms\n",time_ms()-start_time);

    /*start_time = time_ms();
    serial_floyd_warshall(test_matrix,length,path);
    printf("Time for serial approach : %lu ms\n",time_ms()-start_time);
    printf("%s\n",verify_cost(test_matrix,output_matrix,length) ? "TRUE" : "FALSE");*/
...
void parallel_floyd_warshall_workgroup(cl_float** in_matrix, cl_float** out_matrix, cl_uint** out_path, unsigned length, size_t device_num)
{
    build_kernel(device_num);
